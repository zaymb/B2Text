...
- [x] Git 仓库初始化并推送
- [x] Feature 1：静音检测双阈值（能量阈值调优至 0.0003）
- [x] Feature 2：Tab 整合 + 输入源 Toggle（4 Tab → 3 Tab）
- [x] Feature 3：托管模式（BV 自动清理 / 录音自动识别 / 静音暂停分流）
- [x] Feature 5：默认模型改 medium
- [x] Feature 6（部分）：音频波形图 + 分段识别确定进度条 + 状态文字颜色变化
- [x] 全局模型选择 + 关键词提示
- [x] UI 布局优化（去标题、精简静音设置、窗口 800x750）
- [x] 一键清理文件按钮（Tab 1 + Tab 2）
- [x] 静音暂停后继续录制选项
- [x] 去掉录音完成弹窗（段数在界面显示）
- [x] 识别重复检测（连续 2 段重复自动中止）
