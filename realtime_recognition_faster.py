...
class FasterRealtimeRecognizer:
    def __init__(self, model_name="large-v3", device_name=None, initial_prompt="", enable_hallucination_filter=True):
        """
        初始化实时识别器（使用 faster-whisper）
        :param model_name: whisper模型名称 (tiny, base, small, medium, large-v2, large-v3)
        :param device_name: 音频设备名称，如果为None则自动检测
        ...
        if torch.cuda.is_available():
            ...
            device = "cpu"  # faster-whisper 在 Mac 上使用 CPU 更稳定
            compute_type = "int8"
        else:
            device = "cpu"
            compute_type = "int8"
        ...
        print(f"加载模型: {model_name} (首次需要下载，请耐心等待)")
    ...
    def _recognize_audio(self):
        ...
        with open(self.output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"使用模型: faster-whisper large-v3\n")
            f.write("=" * 50 + "\n\n")
        ...
        while self.is_recording or not self.audio_queue.empty():
            try:
                ...
                segments, info = self.model.transcribe(
                    ...
                    best_of=5,  # 采样5个候选
                    temperature=0.0 if self.enable_hallucination_filter else 0.2,
                    ...
                    condition_on_previous_text=False if self.enable_hallucination_filter else True
                )
                ...
                elif text and self.enable_hallucination_filter:
...
def main():
    """主函数，用于测试"""
    print("=" * 50)
    print("实时音频识别（使用faster-whisper + large-v3）")
    print("=" * 50)
    print("\n支持的虚拟音频设备:")
    ...
    print("6. large-v3 (1550M, 最准确，推荐)")

    model_choice = input("请选择 (1-6，默认6): ").strip() or "6"
    model_map = {
        "1": "tiny",
        "2": "base",
        "3": "small",
    ...
    print("\n设置关键词提示（提高识别准确度）:")
    print("示例：")
    print("  - 财经内容: '股票、基金、投资、市场分析'")
    print("  - 技术内容: '编程、软件开发、Python'")
    print("  - 游戏内容: '游戏解说、游戏术语'")
    custom_prompt = input("请输入关键词提示（回车跳过使用默认）: ").strip()

    if custom_prompt:
        # 组合提示词
        full_prompt = f"以下是普通话的句子。这是关于{custom_prompt}的内容。"
    ...
    print(f"\n加载{model_name}模型中...")
    recognizer = FasterRealtimeRecognizer(model_name=model_name, initial_prompt=full_prompt)
